...
// Default is 50. Set to 25 to divide mute/unmute time by 2.
// —————————————————————————————————————————————————————————————————————————————
#define AUDIO_VOLUME_RAMP_STEPS 25
// #define CurveOff345              // Uncomment to use original v3.4.5 polynomial volume curves instead of restored SB curves
// —————————————————————————————————————————————————————————————————————————————

// 🔗 Automatic Dependencies
#ifdef AUDIO_CODEC_M4A
#ifndef AUDIO_CODEC_AAC
#define AUDIO_CODEC_AAC // M4A requires AAC decoder
#endif
#endif

#ifdef AUDIO_CODEC_VORBIS
#ifndef AUDIO_CODEC_OGG
#define AUDIO_CODEC_OGG // Vorbis requires OGG container
#endif
#endif

#ifdef AUDIO_CODEC_OGG
#ifndef AUDIO_CODEC_OPUS
...
public:
    ...
    typedef struct _msg
    { // used in info(audio_info_callback());
...
private:
    ...
    static constexpr const char *const plsFmtStr[5] = {"NONE", "M3U", "PLS", "ASX", "M3U8"}; // playlist format string
    enum : int
    {
        AUDIO_NONE,
    ...
    static constexpr const char *const dataModeStr[8] = {"AUDIO_NONE", "HTTP_RESPONSE_HEADER", "HTTP_RANGE_HEADER", "AUDIO_DATA", "AUDIO_LOCALFILE", "AUDIO_PLAYLISTINIT", "AUDIO_PLAYLISTHEADER", "AUDIO_PLAYLISTDATA"};
    enum : int
    {
        FLAC_BEGIN = 0,
        FLAC_MAGIC = 1,
    ...
    static constexpr const char *const codecname[10] = {"unknown", "WAV", "MP3", "AAC", "M4A", "FLAC", "OPUS", "VORBIS", "OGG"};
    enum : int
    {
        ST_NONE = 0,
        ST_WEBFILE = 1,
    ...
    static constexpr const char *const streamTypeStr[3] = {"NONE", "WEBFILE", "WEBSTREAM"};
    typedef enum
    {
        LEFTCHANNEL = 0,
        RIGHTCHANNEL = 1
...
#pragma GCC diagnostic pop
    ...
    bool m_f_connectionClose = false;     // set in parse HttpResponseHeader
...
public:
    ...
    template <size_t N>
    static const char *safe_arg(const char (&v)[N]) { return v; }
    ...
    template <typename T>
    static auto safe_arg(T &&v) -> decltype(auto) { return std::forward<T>(v); }
    ...
    static bool info(Audio &instance, event_t e, const char *fmt, Args &&...args)
    {
        ...
        auto extract_last_number = [](std::string_view s) -> int32_t
        {
            ...
            while (it != s.begin() && std::isdigit(static_cast<unsigned char>(*(it - 1))))
            {
                --it;
            }
    ...
    template <typename... Args>
    static void AUDIO_LOG_IMPL(uint8_t level, const char *path, int line, const char *fmt, Args &&...args)
    {
...
#define ANSI_ESC_WHITE "\033[37m"
        ...
        while (logStr.contains("/"))
        {
            logStr.remove_before('/', false);
        }
...
struct _HeapGuardSnapshot
{
    ...
    {
        if (!active)
            return; // falls moved / deaktiviert
